    ...
    for i in personnel_list:
        if i['name'] == name:
            if score['p_name'] == name:
                isfollow = True
            returnObj = copy.deepcopy(i)
            returnObj['image'] = str(base64.b64encode(returnObj['image']))
            return JsonResponse({'personnel_info': returnObj, 'isfollow': isfollow})
...
def scored(request):
    ...
    if request.POST:
        for i in range(1, 11):
            score['score_'+str(i)] = int(request.POST['score_'+str(i)])
            score_all += int(request.POST['score_'+str(i)])
